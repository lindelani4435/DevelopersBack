...
app.post('/uploadTeam', async(req, res) =>{
    ...
    try{
        ...
        const allowedExtensions = /jpg|JPG|png|PNG|JPEG|jpeg/;
...
//----------------------------------------------Start_UploadFileRewards---------------------------------------------------------------//


///Insert to teams

app.post('/uploadRewards', async(req, res) =>{
...
try{
	...
	const allowedExtensions = /jpg|JPG|png|PNG|gif|GIF|JPEG|jpeg/;
	...
	const md5 = file.md5;
	const URL = "/rewards/" + md5 + extension;
    await util.promisify(file.mv)("./public" + URL);

    let sql = `INSERT INTO rewards ( picturePath, topic ) VALUES( "${ "http://localhost:9002/public" + URL }", "${topic}" )`;
...
app.listen(process.env.PORT || 9002,() =>{
    ...
    console.log(`server running....${process.env.PORT || 9002}`);
}); 
