...
if (isset($_POST['formData'])) {
      ...
      $financeNames = $_POST['financeNames'];
      $financeSurname = $_POST['financeSurname'];
      $financeEmail = $_POST['financeEmail'];
      $financeNo = $_POST['financeNo'];
      $userType = $_POST['userType'];

      //Form validation section
      if ($financeNames == '' || $financeSurname == '' || $financeEmail == '' || $financeNo == ''|| $userType == '') {
          echo '<script>alert("All Fields are required!!!")</script>';
          echo "<script>location.replace('formFinance.php');</script>";
      }else {
        if (!filter_var($financeEmail,FILTER_VALIDATE_EMAIL)) {
          echo '<script>alert("Enter valid email!!!")</script>';
          echo "<script>location.replace('formFinance.php');</script>";
        }else {
          if (!preg_match('/^[0-9]{10}+$/', $financeNo)) {
            echo '<script>alert("Enter valid phone!!!")</script>';
            echo "<script>location.replace('formFinance.php');</script>";
          }else {
            if (!preg_match('/^[a-zA-Z]+$/', $financeNames)) {
              echo '<script>alert("Enter valid client name!!!")</script>';
              echo "<script>location.replace('formFinance.php');</script>";
            }else {
                if (!preg_match('/^[a-zA-Z]+$/', $financeSurname)) {
                  echo '<script>alert("Enter valid name and surname!!!")</script>';
                  echo "<script>location.replace('formFinance.php');</script>";
                }
                if ($userType == 'Finance') {
                  //Inserting finance data into database
                  $sql = "INSERT INTO finance(financeNames,financeSurname,financeEmail,financeNo,userType)\n
                          VALUES ('$financeNames','$financeSurname','$financeEmail','$financeNo','$userType')";
                  mysqli_query($conn,$sql);

                  echo '<script>alert("Finance information captured!!!")</script>';
                  echo "<script>location.replace('dashboard.php');</script>";

                }else {
                  //Inserting attorney data into database
                  $sql = "INSERT INTO attorney(attorneyNames,attorneySurname,attorneyEmail,attorneyNo,userType)\n
                          VALUES ('$financeNames','$financeSurname','$financeEmail','$financeNo','$userType')";
                  mysqli_query($conn,$sql);

                  echo '<script>alert("Attorney information captured!!!")</script>';
                  echo "<script>location.replace('dashboard.php');</script>";
                }
...
<div class="card1Text">
  ...
  <div class="form-container">
      ...
      <label>Names</label>
        ...
        <label for="roomType">Choose User Type:</label>
        <select name="roomType" id="roomType">
              <option selected disabled value="volvo">---User Type---</option>
